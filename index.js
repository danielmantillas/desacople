...
function sendTurnsToLocalSockets() {
  ['A', 'B'].forEach(team => {
    ...
    const guessed = team === 'A' ? gs.p1.guessedA : gs.p1.guessedB;
    if (!turn || !words.length) return;
    const word = words[idx];
    const wd   = wFind(word);
    io.to('team'+team).emit('p1:turnUpdate', {
      team, turn, wordIndex: guessed.length, scores: gs.scores,
      passes: team === 'A' ? gs.p1.passesA : gs.p1.passesB
    });
    for (const [sid, sock] of io.sockets.sockets) {
...
const RCOLORS = {
  universidad: { bg:'#B5D4F4', tx:'#0C447C' },
  gobierno:    { bg:'#FAC775', tx:'#633806' },
  empresa:     { bg:'#F4C0D1', tx:'#72243E' },
  org_civil:   { bg:'#CECBF6', tx:'#3C3489' },
...
function assignTeam(sid) {
  ...
  p.role = role; p.roleLabel = RLABELS[role]; p.team = team;
  p.color = c.bg; p.textColor = c.tx;
...
function pubState() {
  ...
  Object.entries(gs.players).forEach(([id, p]) => {
    players[id] = {
      id: p.id, name: p.name, role: p.role, roleLabel: p.roleLabel,
      team: p.team, initials: p.initials, color: p.color,
      textColor: p.textColor, isModerator: p.isModerator
    };
  });
...
function nextTurn(team) {
  ...
  if (team === 'A') gs.p1.turnA = { mime, guesser };
  else              gs.p1.turnB = { mime, guesser };
}
...
function startTurn(team) {
  ...
  io.to('moderator').emit('p1:modUpdate', {
    team, word, guessed: guessed.length,
...
function passP3(team, fromId, src) {
  ...
  if (src !== 'timeout') {
    gs.scores[team] -= 3;
    if (team === 'A') p3.passesA++; else p3.passesB++;
  }
...
function calcSub(team) {
  ...
  if (u.size >= 5) s += 40; else if (u.size === 4) s += 20;
  else if (u.size === 3) s += 10; else s -= 10;
  if (roles.includes('comunidad')) s += 20;
  if (roles.includes('org_civil'))  s += 20;
  s -= ghosts.length * 20;
  s -= crosses * 10;
  if (rebounds >= 2) s -= 15;
...
io.on('connection', socket => {
  ...
  socket.on('p2:submit', ({ text, isAuto }) => {
    const p = gs.players[socket.id];
    if (!p || p.role !== 'universidad') return;
    const team = p.team;
  ...
  socket.on('startPhase3', () => {
    ...
    p3.turnA = uA; p3.turnB = uB;
    p3.wordA = p3.wordsA[0]; p3.wordB = p3.wordsB[0];
  ...
  socket.on('p3:sentence', ({ text, nextId }) => {
    try {
      ...
      if (team === 'A') { p3.turnA = nextId; p3.wordA = nw; }
